...
//     }
// };
optimization.runtimeChunk={name:'manifest'};}return{entry:entrypoints,target:target==='server'?'node':'web',externals:target==='server'?[nodeExternals()]:[],output:{filename:target==='server'?'[name].js':'[name].[chunkhash].js',publicPath:'/dist/',...output},resolve:{alias:{__root:process.cwd(),'ha/document':path.resolve(__dirname,'../server/pages/_document')},extensions:['.ts','.tsx','.js','.jsx','.json']},node:{__dirname:true},mode:'development',externals:{},module:{rules:[{test:/\.jsx?$/,exclude:[/node_modules/],use:[// 'thread-loader',
